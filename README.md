...
* quiz.app:macOS用のアプリケーションです
* quiz.exe:windows用のアプリケーションです
* question:問題を記述したcsvファイルを入れておくフォルダです。ここに問題を記載したcsvファイルを置いておきます(執筆方法等は後述)。  
* item:背景画像など、ソフトウェアに必要な画像を入れているフォルダです。
macOSならquiz.appファイルを、windowsならquiz.execを起動してください。  
このアプリの起動にはフォルダの場所がとても重要です。この4つのアイテムは全て一つのフォルダにまとめた状態で、実行用ファイルを起動してください。
